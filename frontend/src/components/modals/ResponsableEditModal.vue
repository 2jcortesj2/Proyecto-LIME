...
// Load responsable data when prop changes
watch(() => props.responsable, (newResponsable) => {
  if (newResponsable) {
    form.value = {
      id: newResponsable.id,
      nombre_completo: newResponsable.nombre_completo || '',
      rol: newResponsable.rol || '',
      email: newResponsable.email || '',
      telefono: newResponsable.telefono || '',
      activo: newResponsable.activo !== undefined ? newResponsable.activo : true
    }
  }
}, { immediate: true })
...
async function submitForm() {
  ...
  try {
    const response = await fetch(buildApiUrl(`/api/responsables/${form.value.id}/`), {
      method: 'PUT',
      headers: {
