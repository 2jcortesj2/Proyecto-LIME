...
def reprogramar_mantenimiento(request, pk):
    """
    Reprograma la fecha del próximo mantenimiento de un equipo.
    ...
    if not nueva_fecha_str:
        return Response({'error': 'Se requiere una nueva fecha'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    if hasattr(equipo, 'informacion_metrologica'):
        ...
        return Response({'message': 'Mantenimiento reprogramado exitosamente', 'nueva_fecha': nueva_fecha})
    else:
        return Response({'error': 'El equipo no tiene información metrológica asociada'}, status=status.HTTP_400_BAD_REQUEST)